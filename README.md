...
| 首页 Dashboard | 「合同数量」+「合同总金额」两张统计卡片,下方内嵌「项目管理」区块 |
...
| 合同管理 | 合同录入/编辑、**金额调整**(新金额不能低于已发生资金/发票合计)、合同类型(销售/采购);**验收单据**(验收日期 + 验收单文件);**删除合同级联清理**其下全部资金/发票/验收单据/附件;无「状态」概念、无变更履历 |
| 资金记录 | 收款/付款方向由合同类型自动推导;「待确认 → 确认到账/作废」 |
| 发票记录 | 销项/进项由合同类型自动推导;支持发票文件上传;**可直接删除单张发票**;无核销、无认证抵扣、无红冲 |
...
- 字典类枚举(合同类型/项目状态/发票状态等)走 `sys_dict` 表,`/api/dict/list` 接口下发,前端不写死。
